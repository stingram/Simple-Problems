...
# now we create a renderer_silhouette renderer. This is differentiable
# for rendering silhouette images. It is mainly used for silhouette fitting
blend_params = BlendParams(sigma=1e-4,gamma=1e-4)
raster_settings = RasterizationSettings(
    image_size=256,
    blur_radius=np.log(1. / 1e-4 - 1.) * blend_params.sigma, faces_per_pixel=100,
)
...
# next we create a renderer_textured renderer, another differentiable renderer
# used for rendering RGB images
sigma = 1e-4
...
# next we define a camera position and corresponding rotation and position
# This will be a camera position where the observed image is taken
# We optimize the camera orientation and position instead of the object orientation
# and position. We also assume the camera is always pointing toward the object, thus
# we only need to optimize the camera position
distance = 3
elevation = 50.0
azimuth = 0.0
R,T = look_at_view_transform(distance, elevation, azimuth, device=device)

# next we create the observed images and save them to target_silhouette.png
# and target_rgb.png. The images aare also stored in the silhouette and image_ref
# variables
...
class Model(nn.Module):
    def __init__(self, meshes, renderer_silhouette, renderer_textured, image_ref,
    ...
    def forward(self):
        ...
        R = look_at_rotation(self.camera_position[None,:], device=self.device) # shape (1, 3, 3)
